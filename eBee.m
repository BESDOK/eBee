...
An Artificial Bee Colony Algorithm with Stochastic Mask-Driven Perturbation and Elite Guidance for Numerical Optimization : eBee (ELİTE-GUIDENCE BEE ALGORITHM FOR NUMERICAL FUNCTION OPTIMIZATION)
Besdok, E. 2026, Kayseri, TÜRKİYE


eBee is a variant of the ABC algorithm developed to solve real-valued numerical optimization problems.
eBee is designed as a single-objective, bounded, swarm-based, iterative population evolution-based evolutionary global minimizer algorithm.
eBee utilizes three distinct phases while searching for the global minimum of a given problem: Employed-Bee search, Onlooker-Bee search, and Scout-Bee search phases.

%}


function out = eBee(ObjFnc, userdata, N, D, lb, ub, MaxCycle)
CSize=2;
...
% In the eBee algorithm, the Flowers vector with the best quality level currently held by FlowerField is the best nectar source.
% In eBee, the best nectar source corresponds to the global best solution vector (i.e., gBest).
...
for iteration = 1:MaxCycle
    ...
    % Since this process enables the FlowerField vectors (i.e., Flowers vectors) stored at the beginning of the iteration to be evolved further in a different way, it contributes to the exploitation (local search) capability of eBee.
    ...
    % Anlık olarak bulunulan Meadow kaynaklarını kullanmaya devam etme olasılığı 50%
    ...
    assignin('base','eBee_out',out); % workspace reporter
    
	
	% if ceil(10^16*gBestVal)==ceil(10^16*kBestVal)
        % ksay=ksay+1;
        % if ksay>=10000, return; end
    % else, ksay=0; kBestVal=gBestVal; 
    % end

% if ceil(10^16*abs(gBestVal))/10^16 == 0, return, end
